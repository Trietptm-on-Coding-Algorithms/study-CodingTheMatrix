...
def setitem(v,k,val):
    ...
    setitem(v,k,val) should set the value for key d even if d
...
def equal(u,v):
    ...
    for d in u.D:
        if d in u.f and d in v.f:
            isSame = (u.f[d]==v.f[d])
        elif d not in v.f and d in u.f:
            isSame = (u.f[d]==0)
        elif d in v.f and d not in u.f:
            isSame = (v.f[d]==0)
